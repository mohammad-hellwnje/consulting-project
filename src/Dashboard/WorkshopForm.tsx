      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-6">
        <div className="flex items-center gap-2">
          <div className="w-1/2">
            ...
            <Input name="title" value={formData.title} onChange={handleChange} placeholder="اضف عنوان للورشة" required />
          </div>
          <div className="w-1/2">
            <Label label="السعر" />
            <Input name="price" type="number" value={formData.price} onChange={handleChange} placeholder="اضف سعر الورشة" required />
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          <div className="w-1/2">
            ...
            <Input name="seats" type="number" value={formData.seats} onChange={handleChange} placeholder="اضف عدد المقاعد" required />
          </div>
        </div>
        ...
        <div className="flex flex-col h-full lg:flex-row justify-between gap-6">
          <div className="w-1/2">
            <Label label="الوصف" />
            <textarea name="description" value={formData.description} onChange={handleChange} rows={5} className="w-full rounded-md border p-3 resize-none" placeholder="اضف شرح يوصّف الورشة" required />
          </div>
            ...
            <div className="border-2 h-[158px] flex flex-col items-center justify-center border-dashed rounded-lg p-4 text-center hover:border-[#b69cf0] transition-all bg-gray-50">
              {preview ? (
                <div className="relative w-full h-full">
                  ...
                  <button type="button" onClick={() => { setImageFile(null); setPreview(null); }} className="absolute top-2 right-2 bg-red-500 text-white rounded-full px-2 py-1 text-xs">إزالة</button>
                </div>
              ) : (
        ...
        <div className="flex justify-end">
          <button type="submit" disabled={loading} className="px-6 py-2 bg-primary hover:bg-[#a58ae8] text-white font-medium rounded-lg transition-colors">
