...
import {
  getAllInPersonCourseBookings,
  confirmInPersonCourseBooking,
  rejectInPersonCourseBooking,
  InPersonCourseBooking,
} from "../services/inPersonCourseBookingsApi";

const InPersonCourseBookingsTable: React.FC = () => {
  const [bookings, setBookings] = useState<InPersonCourseBooking[]>([]);
  const [loading, setLoading] = useState(true);
  const [selectedBooking, setSelectedBooking] = useState<InPersonCourseBooking | null>(
    null
  );
  ...
  const fetchBookings = async () => {
    setLoading(true);
    try {
      const data = await getAllInPersonCourseBookings();
  ...
  const handleConfirm = (booking: InPersonCourseBooking) => {
    setSelectedBooking(booking);
    setModalType("confirm");
  };

  const handleReject = (booking: InPersonCourseBooking) => {
    setSelectedBooking(booking);
    setModalType("reject");
  };

  const handleView = (booking: InPersonCourseBooking) => {
    setSelectedBooking(booking);
    setModalType("view");
  };
  ...
  const confirmAction = async () => {
    ...
    try {
      if (modalType === "confirm") {
        await confirmInPersonCourseBooking(selectedBooking._id);
        alert("✅ تم تأكيد الحجز بنجاح!");
      } else if (modalType === "reject") {
        await rejectInPersonCourseBooking(selectedBooking._id);
        alert("✅ تم رفض الحجز!");
      }
      setModalType(null);
      fetchBookings();
  ...
  return (
    <div className="p-6 bg-white rounded-lg shadow-md">
      <h2 className="text-2xl font-bold mb-6 text-gray-800">إدارة حجوزات الدورات</h2>
      ...
      >
        {selectedBooking && (
          <div className="space-y-3 text-gray-700">
            ...
            <p>
              <strong>الدورة:</strong> {selectedBooking.course?.title}
            </p>
            <p>
              <strong>التاريخ:</strong>{" "}
              {new Date(selectedBooking.course?.date || "").toLocaleDateString(
                "ar-EG"
              )}
            </p>
            <p>
...
export default InPersonCourseBookingsTable;
