...
import { gitWorkShop } from "../services/workshopsApi"; // استدعاء الدالة الجاهزة
...
const WorkshopsTable: React.FC = () => {
  ...
  const handleView = (workshop: Workshop) => { setSelectedWorkshop(workshop); setModalType("view"); };
  const handleEdit = (id: string) => navigate(`/dashboard/editworkshop/${id}`);
  const handleDelete = (workshop: Workshop) => { setSelectedWorkshop(workshop); setModalType("delete"); };
  ...
  return (
    <div className="mt-8">
      <table className="min-w-full bg-white rounded-2xl shadow-md overflow-hidden border border-gray-200">
        <thead className="bg-[#f3f2f7]">
          <tr>
            ...
            <th className="py-3 px-6 text-right text-gray-700 font-semibold">تاريخ الورشة</th>
        ...
        <tbody>
          {workshops.map(workshop => (
            <tr key={workshop._id} className="border-b last:border-none hover:bg-[#f9f8fc] transition-colors">
              ...
              <td className="py-3 px-6 text-gray-500 text-sm">
                {workshop.startTime && workshop.endTime ? `${workshop.startTime} - ${workshop.endTime}` : "-"}
              </td>
              <td className="py-3 px-6 text-center flex gap-3 justify-center">
                <button onClick={() => handleView(workshop)} className="text-blue-500 hover:text-blue-700" title="عرض"><FaEye /></button>
                <button onClick={() => handleEdit(workshop._id)} className="text-yellow-500 hover:text-yellow-700" title="تعديل"><FaEdit /></button>
                <button onClick={() => handleDelete(workshop)} className="text-red-500 hover:text-red-700" title="حذف"><FaTrash /></button>
              </td>
            </tr>
          ))}
        </tbody>
      ...
      {/* مودال العرض */}
      <Modal title="تفاصيل الورشة" isOpen={modalType === "view"} onClose={() => setModalType(null)}>
        {selectedWorkshop && (
          <div className="space-y-2 text-gray-700">
            ...
            <p><strong>التاريخ:</strong> {selectedWorkshop.date ? new Date(selectedWorkshop.date).toLocaleDateString("ar-EG") : "-"}</p>
            <p><strong>الوقت:</strong> {selectedWorkshop.startTime && selectedWorkshop.endTime ? `${selectedWorkshop.startTime} - ${selectedWorkshop.endTime}` : "-"}</p>
            {selectedWorkshop.image && <img src={`https://your-domain.com/${selectedWorkshop.image}`} alt={selectedWorkshop.title} className="w-32 h-20 object-cover mt-2 rounded-md"/>}
          </div>
        )}
      </Modal>
      ...
      <Modal title="تأكيد الحذف" isOpen={modalType === "delete"} onClose={() => setModalType(null)}>
        <p className="text-gray-700 mb-4">هل أنت متأكد من حذف الورشة "<strong>{selectedWorkshop?.title}</strong>"؟</p>
